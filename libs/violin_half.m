...
%             h(i)=fill([F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
            ...
            % h(i)=fill([F(:,i)+x(i);flipud(x(i)*ones(size(F(:,i))))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        end
    else
        if setX == 0
            h(i)=fill([F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
            %claude fail: h(i)=fill([F(:,i)+x(i);flipud(x(i)*ones(size(F(:,i))))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);

            % white box
            ylims = [min(U(:,i)) max(U(:,i))];
            % i = which violin
            % first part: which x axis, second part, which y axis
            patch([i-min(F(:,i)), i-max(F(:,i)), i-max(F(:,i)), i-min(F(:,i))], [ylims(1), ylims(1), ylims(2), ylims(2)],'w', 'EdgeColor', 'none');

            % black line
%             hold on
%             plot([i, i], [ylims(1), ylims(2)], 'k--', 'LineWidth', 1);
